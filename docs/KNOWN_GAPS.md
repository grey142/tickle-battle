...
- **Walkable 3D plaza hub:** first-person WASD/LS + look; axis-separated wall slide with extended jamb slack + jamb-tip unjam (Home/Shop/Arena); walk up to **Arena** (or click its door) to start Amateur Team Quick — near-door trim glow + soft point-light pulse + threshold sills + sign pulse on **all three doors** (Home/Shop match Arena pulse; Arena slight CTA lead) + pulsing floor pads; Leave / results return to plaza with profile reloaded from disk and match ephemeral wiped (coins/XP/Look/loadout preserved)
...
- Walkable plaza ships with over-shoulder third-person mannequin on multi-frame idle sheets plus Home/Shop/Arena near-door soft glow/pulse + threshold sills + jamb-tip unjam wall-slide + hardened Leave/Return (ephemeral wipe); remaining hub polish still TBD
