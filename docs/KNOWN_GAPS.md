...
- Walkable plaza ships with over-shoulder third-person mannequin on multi-frame idle sheets plus Home/Shop/Arena near-door soft glow/pulse + inner/outer sills + biased-smoothstep approach + stronger frame lift + deeper jamb-tip unjam wall-slide + hardened Leave/Return (ephemeral wipe + ~0.65s confirm grace); remaining hub polish still TBD
- Painterly four-clip production anims and blendshapes still missing (tickle five-frame deepened A-pose cycle + intensity pick, run four-frame oil-grade adaptations past #41, laugh five-frame stamina-weighted cycle + blend deepen ship; full painterly anims TBD).
...
Painterly s0–s3 stage stills plus **five-frame** stamina-weighted cycle (`assets/binds/laugh/frames/` f0–f4) for Amateur 12 + Elara (f4 derived from f2/f3; metal-free; capsule face-card only). Ticklee billboards use **tighter** stamina→frame windows past #46 (high soft-locks f0–f1 earlier; mid f1–f2; low f2–f3; peak f3–f4 / f4 soft-lock) with more aggressive billRate×stamina FPS; stage binds bump rate/amp/lean/billShake/blend again (cap 1.0); Humanoid squirm face-card (jaw/cheek/eye/brow + head tilt) another clear morph notch past #46; hub Look / plaza laugh preview harder (~3% stamina + stronger CSS morph). Remaining: true morph-target mesh blendshapes if a non-capsule head ships later — still-adaptation only; do not invent mesh blendshapes.
...
Painterly four-frame run cycles for Amateur 12 + Elara under `assets/binds/run/frames/` (f0 = A-pose still; f1–f3 **deeper opposite-leg stride punch** bob/sway/squash + oil-paint grade + vignette past #41 + bumped bind rate/amp/lean/billBob). Humanoid loco + billboard bob strengthened again. AI billboards + FP portrait cycle at loco `billRate` (walk uses walk.billRate). Remaining: true four-clip production anims/blendshapes beyond still swaps.
