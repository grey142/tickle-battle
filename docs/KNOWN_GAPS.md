...
- Painterly four-clip production anims and blendshapes still missing (tickle five-frame deepened A-pose cycle + intensity pick past #96, run four-frame oil-grade adaptations past #53, laugh five-frame stamina-weighted cycle + blend deepen past #97 ship; full painterly anims TBD).
...
- Full licensed SFX library still TBD (and out of scope — procedural/CC0 only); Team Quick UI/combat stingers use deepened layered samples + oscillator fallbacks (countdown last-3s widest rate ladder + stronger vanish soft-mode, deepest vanish whoosh vs brightest reappear ping past #96, soft unlock chirp)
- No jump (bible); ramp height is a groundY sample

## Laugh production clips (Support)
Painterly s0–s3 stage stills plus **five-frame** stamina-weighted cycle (`assets/binds/laugh/frames/` f0–f4) for Amateur 12 + Elara (f4 derived from f2/f3; metal-free; capsule face-card only). Ticklee billboards use **tighter** stamina→frame windows past #97 (high soft-locks f0–f1 earlier; mid f1–f2; low f2–f3; peak f3–f4 / f4 soft-lock) with more aggressive billRate×stamina FPS; stage binds bump rate/amp/lean/billShake/blend again (cap 1.0); Humanoid squirm face-card (jaw/cheek/eye/brow + head tilt) another clear morph notch past #97; hub Look / plaza laugh preview harder (~0.00000023% stamina + stronger CSS morph). Still-adaptation only; do not invent mesh blendshapes. Remaining: true morph-target mesh blendshapes if a non-capsule head ships later.

## Tickle production polish (Support 4)
Five-frame tickle cycles for Amateur 12 + Elara under `assets/binds/tickle/frames/` (f0 = A-pose still; f1–f4 deepened lean/reach/bob with stronger pose deltas + oil-paint grade/vignette). AI tickler billboards + FP portrait use **intensity-weighted** frame windows with tighter band edges + heavier peak f4 bias past #96 (victim stamina + clearer pack-size boost → harder f3/f4; stronger contrast + asymmetric smooth); FP arms use `tickleBind` rate/amps (no hardcoded `sin(t*28)`); Humanoid tickle motion strengthened. Vanish/reappear stingers + white rim flash deepened another notch; countdown leave stays reward-free / profile-safe. Deferred: true multi-clip mesh anims / blendshapes beyond still adaptations.
