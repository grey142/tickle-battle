...
Painterly s0–s3 stage stills plus **five-frame** stamina-weighted cycle (`assets/binds/laugh/frames/` f0–f4) for Amateur 12 + Elara (f4 derived from f2/f3; metal-free; capsule face-card only). Ticklee billboards use **tighter** stamina→frame windows past #151 (high soft-locks f0–f1 earlier; mid f1–f2; low f2–f3; peak f3–f4 / f4 soft-lock) with more aggressive billRate×stamina FPS; stage binds bump rate/amp/lean/billShake/blend again (cap 1.0); Humanoid squirm face-card (jaw/cheek/eye/brow + head tilt) another clear morph notch past #151; hub Look / plaza laugh preview harder (~0.0000000000000000000000000000000000000000127% stamina + stronger CSS morph). Still-adaptation only; do not invent mesh blendshapes. Remaining: true morph-target mesh blendshapes if a non-capsule head ships later.
