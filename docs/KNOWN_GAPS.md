...
Painterly s0–s3 stage stills plus **five-frame** stamina-weighted cycle (`assets/binds/laugh/frames/` f0–f4) for Amateur 12 + Elara (f4 derived from f2/f3; metal-free; capsule face-card only). Ticklee billboards use **tighter** stamina→frame windows past #31 (high soft-locks f0–f1; mid opens f1–f2; low/peak biases f3–f4 earlier) with more aggressive billRate×stamina FPS; stage binds bump rate/amp/lean/billShake/blend (cap 1.0); Humanoid squirm face-card (jaw/cheek/eye/brow + head tilt) another clear notch past #31; hub Look / plaza laugh preview harder (~16% stamina + stronger CSS morph). Remaining: true morph-target mesh blendshapes if a non-capsule head ships later.
