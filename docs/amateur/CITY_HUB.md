...
4. Match end → post-match results → plaza with updated coins (Leave during countdown / Return both reload persisted profile, wipe match ephemeral, exit pointer-lock, and ~0.85s pad confirm grace so Arena does not instantly re-queue).
