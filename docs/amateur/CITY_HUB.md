...
4. Match end → post-match results → plaza with updated coins (Leave during countdown / Return both reload persisted profile, wipe match ephemeral, exit pointer-lock, and ~1.25s pad confirm grace and no reward write so Arena does not instantly re-queue).
