...
- `assets/binds/run/frames/{Slug}_f0.jpg` … `_f3.jpg` — four-frame run cycle (f0 = A-pose still; f1–f3 = further deepened bob/sway/squash stride beyond #33)
...
- AI woman billboards bob (stronger sway/squash) while walking/running **and** cycle the four production frames at loco `billRate` FPS (walk uses walk.billRate); restore keyed A-pose still when idle/busy. Humanoid 3D stride bob/sway/squash further deepened beyond #33.
