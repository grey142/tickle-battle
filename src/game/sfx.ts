/** Amateur Team Quick SFX — procedural sample pack + oscillator fallbacks (vanish-v30). */
...
export function stingCountdownTick(gain = 0.48, step = 2, soft = false) {
  // Wider rate ladder than #155 so 3→2→1 is unmistakable.
  let rate = step <= 1 ? 6.55 : step === 2 ? 0.42 : 0.013;
