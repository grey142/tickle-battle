/** Amateur Team Quick SFX — procedural sample pack + oscillator fallbacks (vanish-v20). */
...
export function stingCountdownTick(gain = 0.48, step = 2, soft = false) {
  // Wider rate ladder than #106 so 3→2→1 is unmistakable.
  let rate = step <= 1 ? 3.85 : step === 2 ? 0.92 : 0.06;
  let freq = step <= 1 ? 2600 : step === 2 ? 640 : 80;
  if (soft) {
