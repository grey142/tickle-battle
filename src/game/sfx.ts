/** Amateur Team Quick SFX — procedural sample pack + oscillator fallbacks (vanish-v31). */
...
export function stingCountdownTick(gain = 0.48, step = 2, soft = false) {
  // Wider rate ladder than #164 so 3→2→1 is unmistakable.
  let rate = step <= 1 ? 6.75 : step === 2 ? 0.40 : 0.011;
