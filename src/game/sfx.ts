/** Amateur Team Quick SFX — procedural sample pack + oscillator fallbacks (vanish-v21). */
...
export function stingCountdownTick(gain = 0.48, step = 2, soft = false) {
  // Wider rate ladder than #109 so 3→2→1 is unmistakable.
  let rate = step <= 1 ? 4.15 : step === 2 ? 0.85 : 0.05;
  let freq = step <= 1 ? 2800 : step === 2 ? 600 : 72;
  if (soft) {
