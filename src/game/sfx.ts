/** Amateur Team Quick SFX — procedural sample pack + oscillator fallbacks (vanish-v28). */
...
export function stingCountdownTick(gain = 0.48, step = 2, soft = false) {
  // Wider rate ladder than #147 so 3→2→1 is unmistakable.
  let rate = step <= 1 ? 6.15 : step === 2 ? 0.46 : 0.017;
