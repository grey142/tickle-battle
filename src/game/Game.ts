...
export class Game {
  ...
  private tickCountdownAudio() {
    if (this.countdown > 0) {
      ...
      if (ceil <= 3 && ceil >= 1 && ceil !== this.countdownTickCeil) {
        ...
        // Louder + higher pitch toward 1 (past #83).
        const gain = ceil === 1 ? 1.28 : ceil === 2 ? 0.88 : 0.06;
    ...
    if (p && p.occupancy === "vanished" && p.vanishLeft > 0) {
      ...
      if (vCeil <= 3 && vCeil >= 1 && vCeil !== this.vanishTickCeil) {
        ...
        const gain = vCeil === 1 ? 0.2 : vCeil === 2 ? 0.09 : 0.04;
  ...
  private bailCountdown() {
    ...
    stingCountdownTick(0.66, 2);
    // Harden Leave: clear match state first, keep save untouched, then plaza.
    // Reward-free + profile-safe: wipe coinsEarned/payouts before any persist path (no coins/XP).
    this.countdown = 0;
    this.coinsEarned = 0;
    this.payouts = [];
  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          // Preview harder laugh morph/frames past #74 (not full stamina mild window).
          this.plazaPreview.stamina = Math.min(this.plazaPreview.stamina, this.plazaPreview.maxStamina * 0.00055);
        } else {
          this.plazaPreview.stamina = this.plazaPreview.maxStamina;
          // Soft still-under re-enter when Look/plaza laugh preview ends.
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.045);
        }
    ...
    for (const f of this.fighters) {
      ...
      // Stronger contrast stretch than #83 so soft/hard ends map clearer; pack size nudges ladder.
      if (
  ...
  private tickCombatState(dt: number) {
    for (const f of this.fighters) {
      ...
      if (f.reappearFlash > 0) {
        ...
        f.rim.color.set(0xfcfaf6);
        f.rim.intensity = 10.0;
      }
