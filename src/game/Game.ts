...
export class Game {
  ...
  private tickCountdownAudio() {
    if (this.countdown > 0) {
      ...
      if (ceil <= 3 && ceil >= 1 && ceil !== this.countdownTickCeil) {
        ...
        // Louder + higher pitch toward 1 (past #51).
        const gain = ceil === 1 ? 0.96 : ceil === 2 ? 0.64 : 0.26;
    ...
    if (p && p.occupancy === "vanished" && p.vanishLeft > 0) {
      ...
      if (vCeil <= 3 && vCeil >= 1 && vCeil !== this.vanishTickCeil) {
        ...
        const gain = vCeil === 1 ? 0.4 : vCeil === 2 ? 0.28 : 0.18;
  ...
  private tickHubWalk(dt: number) {
    ...
    if (this.hubNear) {
      ...
      // Penta-biased smoothstep past #53 — snappier near-door CTA / Leave feel.
      ...
      this.plaza.setNearStrength(quad * quad * (3 - 2 * quad));
    } else {
      this.plaza.setNearStrength(0);
    }
  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          ...
          this.plazaPreview.stamina = Math.min(this.plazaPreview.stamina, this.plazaPreview.maxStamina * 0.01);
        } else {
          this.plazaPreview.stamina = this.plazaPreview.maxStamina;
          // Soft still-under re-enter when Look/plaza laugh preview ends.
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.22);
        }
    ...
    for (const f of this.fighters) {
      ...
      // Stronger contrast stretch than #51 so soft/hard ends map clearer.
      if (
        ...
        if (v && v.maxStamina > 0) {
          ...
          const contrasted =
            raw < 0.5 ? 0.5 * Math.pow(raw * 2, 1.96) : 1 - 0.5 * Math.pow((1 - raw) * 2, 1.96);
