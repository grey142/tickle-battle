...
export class Game {
  ...
  private tickHubWalk(dt: number) {
    ...
    if (this.hubNear) {
      ...
      // Hepta-biased smoothstep past #60 — snappier near-door CTA / Leave feel.
      ...
      this.plaza.setNearStrength(hexa * hexa * (3 - 2 * hexa));
    } else {
      this.plaza.setNearStrength(0);
    }
  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          // Preview harder laugh morph/frames past #62 (not full stamina mild window).
          this.plazaPreview.stamina = Math.min(this.plazaPreview.stamina, this.plazaPreview.maxStamina * 0.0055);
        } else {
