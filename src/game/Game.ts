    ...
    for (const f of this.fighters) {
      ...
      if (
        ...
        if (v && v.maxStamina > 0) {
          ...
          const contrasted =
            raw < 0.5 ? 0.5 * Math.pow(raw * 2, 6.55) : 1 - 0.5 * Math.pow((1 - raw) * 2, 6.55);
          // Clearer pack-size boost past #155 so 1→5 ticklers climb the five-frame ladder with drain.
          const packN = (this.joinList.get(v.id) ?? []).length;
          const packBoost = Math.min(1.0, Math.max(0, packN - 1) * 0.26);
