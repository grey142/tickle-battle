  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          // Preview harder laugh morph/frames past #167 (not full stamina mild window).
          this.plazaPreview.stamina = Math.min(this.plazaPreview.stamina, this.plazaPreview.maxStamina * 0.000000000000000000000000000000000000000000000000000139);
        } else {
          this.plazaPreview.stamina = this.plazaPreview.maxStamina;
          // Soft still-under re-enter when Look/plaza laugh preview ends.
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.00072);
        }
    ...
    for (const f of this.fighters) {
      ...
      if (
        ...
        if (v && v.maxStamina > 0) {
          ...
          const contrasted =
            raw < 0.5 ? 0.5 * Math.pow(raw * 2, 6.95) : 1 - 0.5 * Math.pow((1 - raw) * 2, 6.95);
          // Clearer pack-size boost past #169 so 1→5 ticklers climb the five-frame ladder with drain.
          const packN = (this.joinList.get(v.id) ?? []).length;
          const packBoost = Math.min(1.0, Math.max(0, packN - 1) * 0.28);
