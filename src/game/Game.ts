...
export class Game {
  ...
  private tickCountdownAudio() {
    if (this.countdown > 0) {
      ...
      if (ceil <= 3 && ceil >= 1 && ceil !== this.countdownTickCeil) {
        ...
        // Louder + higher pitch toward 1 (past #57).
        const gain = ceil === 1 ? 0.98 : ceil === 2 ? 0.66 : 0.22;
    ...
    if (p && p.occupancy === "vanished" && p.vanishLeft > 0) {
      ...
      if (vCeil <= 3 && vCeil >= 1 && vCeil !== this.vanishTickCeil) {
        ...
        const gain = vCeil === 1 ? 0.36 : vCeil === 2 ? 0.24 : 0.14;
  ...
  private tickHubWalk(dt: number) {
    ...
    if (this.hubNear) {
      ...
      // Hexa-biased smoothstep past #56 — snappier near-door CTA / Leave feel.
      ...
      this.plaza.setNearStrength(penta * penta * (3 - 2 * penta));
    } else {
      this.plaza.setNearStrength(0);
    }
  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          ...
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.18);
        }
    ...
    for (const f of this.fighters) {
      ...
      // Stronger contrast stretch than #57 so soft/hard ends map clearer.
      if (
        ...
        if (v && v.maxStamina > 0) {
          ...
          const contrasted =
            raw < 0.5 ? 0.5 * Math.pow(raw * 2, 2.1) : 1 - 0.5 * Math.pow((1 - raw) * 2, 2.1);
