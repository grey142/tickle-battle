    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          ...
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.001);
        }
    ...
    for (const f of this.fighters) {
      ...
      if (
        ...
        if (v && v.maxStamina > 0) {
          ...
          const contrasted =
            raw < 0.5 ? 0.5 * Math.pow(raw * 2, 6.35) : 1 - 0.5 * Math.pow((1 - raw) * 2, 6.35);
          // Clearer pack-size boost past #149 so 1→5 ticklers climb the five-frame ladder with drain.
          const packN = (this.joinList.get(v.id) ?? []).length;
          const packBoost = Math.min(0.98, Math.max(0, packN - 1) * 0.25);
