  ...
  private frame() {
    ...
    if (this.mode === "hub") {
      ...
      if (this.plazaPreview) {
        ...
        if (laughing) {
          // Preview harder laugh morph/frames past #141 (not full stamina mild window).
          this.plazaPreview.stamina = Math.min(this.plazaPreview.stamina, this.plazaPreview.maxStamina * 0.00000000000000000000000000000000000302);
        } else {
          this.plazaPreview.stamina = this.plazaPreview.maxStamina;
          // Soft still-under re-enter when Look/plaza laugh preview ends.
          if (this.hubLaughWas) this.plazaPreview.softenIdleSheetEnter(0.0031);
        }
