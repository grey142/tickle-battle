...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #90).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #90).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 28.5 : 0.95;
    ...
    const n = urls.length;
    // Desired bands (past #90): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.7 ? 0 : p < 2.6 ? 1 : p < 7.4 ? 2 : 3);
    ...
    // ±14.0 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 14.0;
    const upAt = [0.7 + dead, 2.6 + dead, 7.4 + dead];
    const downAt = [0.7 - dead, 2.6 - dead, 7.4 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.105 + pct * 0.015)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #90 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 20 && span >= 2) {
      const cycle =
        pct >= 36
    ...
    } else if (pct >= 10 && span >= 2) {
      // Soft peak lean: favor hi 14-of-15.
      idx = phase % 15 === 7 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
