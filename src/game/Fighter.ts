...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #154: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 0.567;
  ...
  softenIdleSheetEnter(cap = 0.0016) {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #154: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 84);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #151: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.00000000000000000000000000000000000000960) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.00000000000000000000000000000000000000149) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000000000000000000000000000000000000000208) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000000000000000000000000000000000000000190) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(73871, Math.round((stageParams?.billRate || 14) * (82.51 + (100 - pct) * 45.14)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #151: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 861.19 * pulse + s * shake * 4549.99;
      const morphH = 1 + jaw * 987.11 * pulse - eye * 656.11 * pulse - s * shake * 2400.38;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 6201.14 + s2 * cheek * 358.39;
      y = BILL_Y + Math.abs(s2) * shake * 2975.21 + jaw * 358.39 * pulse;
      rot = s2 * shake * 5822.21 + s * cheek * 377.19;
      ...
      const b = Math.sin(t * rate) * idle.breatheAmp * 5.42;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 3.45;
