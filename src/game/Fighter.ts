...
export class Fighter {
  ...
  softenIdleSheetEnter(cap = 0.1) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #67: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 1.55;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #67: smootherstep × ease³ — longer readable mid-frame hold.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const e = s * s * (3 - 2 * s);
    const e2 = e * e * (3 - 2 * e);
    const blend = e2 * e2 * (3 - 2 * e2);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #70: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    const fps = Math.max(36, Math.round((stageParams?.billRate || 14) * (1.15 + (100 - pct) * 0.04)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #65).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #65).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 17.5 : 1.95;
    ...
    const n = urls.length;
    // Desired bands (past #65): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 3 ? 0 : p < 9 ? 1 : p < 20 ? 2 : 3);
    ...
    // ±9.0 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 9.0;
    const upAt = [3 + dead, 9 + dead, 20 + dead];
    const downAt = [3 - dead, 9 - dead, 20 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.22 + pct * 0.0104)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #65 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 44 && span >= 2) {
      const cycle =
        pct >= 60
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 28 && span >= 2) {
      // Soft peak lean: favor hi 8-of-9.
      idx = phase % 9 === 3 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #70: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.52 * pulse + s * shake * 2.75;
      const morphH = 1 + jaw * 0.58 * pulse - eye * 0.37 * pulse - s * shake * 1.42;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 3.7 + s2 * cheek * 0.21;
      y = BILL_Y + Math.abs(s2) * shake * 2.45 + jaw * 0.21 * pulse;
      rot = s2 * shake * 3.35 + s * cheek * 0.225;
      ...
      // Past #67: idle-only (longer crossfade / softer re-enter / breathe). No laugh/tickle/persist schema.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 1.92;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.18;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 2.0 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 1.55 +
        Math.sin(t * rate * 0.17) * shift * 0.78;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 1.8 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.05);
      h = BILL_H * (1 + (b + b2) * 1.45);
      x = s;
      y = BILL_Y + b + b2 * 1.05;
      rot = s * 1.08;
