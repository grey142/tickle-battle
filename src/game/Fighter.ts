...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #118: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 1.85;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #118: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 1.85;
  ...
  softenIdleSheetEnter(cap = 0.010) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #118: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.36;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #118: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 40);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Past #118: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 3.17;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 2.01;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 3.24 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 2.63 +
        Math.sin(t * rate * 0.17) * shift * 1.41;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.98 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.74);
      h = BILL_H * (1 + (b + b2) * 2.29);
      x = s;
      y = BILL_Y + b + b2 * 1.74;
      rot = s * 1.74;
