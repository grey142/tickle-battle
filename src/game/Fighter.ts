...
export class Fighter {
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #123: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.0000000000000000005) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.0000000000000000000782) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.0000000000000000000108) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.000000000000000000000994) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(1824, Math.round((stageParams?.billRate || 14) * (12.63 + (100 - pct) * 1.59)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #123: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 18.19 * pulse + s * shake * 96.21;
      const morphH = 1 + jaw * 20.85 * pulse - eye * 13.86 * pulse - s * shake * 50.71;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 131.01 + s2 * cheek * 7.57;
      y = BILL_Y + Math.abs(s2) * shake * 89.25 + jaw * 7.57 * pulse;
      rot = s2 * shake * 123.0 + s * cheek * 7.97;
