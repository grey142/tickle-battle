...
export class Fighter {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    this.idleSheetEnter = Math.min(1, this.idleSheetEnter + dt * 3.8);
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #49: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    const fps = Math.max(17, Math.round((stageParams?.billRate || 14) * (0.52 + (100 - pct) * 0.018)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #45).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #45).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 11.8 : 3.6;
    ...
    const n = urls.length;
    // Desired bands (past #45): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 8 ? 0 : p < 18 ? 1 : p < 36 ? 2 : 3);
    ...
    // ±5.8 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 5.8;
    const upAt = [8 + dead, 18 + dead, 36 + dead];
    const downAt = [8 - dead, 18 - dead, 36 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.3 + pct * 0.008)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #45 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 60 && span >= 2) {
      const cycle =
        pct >= 76
          ? [hi, hi, hi, hi, hi, lo, hi, hi]
          : [hi, hi, hi, hi, lo, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 44 && span >= 2) {
      // Soft peak lean: favor hi 4-of-5.
      idx = phase % 5 === 2 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #49: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.22 * pulse + s * shake * 1.28;
      const morphH = 1 + jaw * 0.26 * pulse - eye * 0.155 * pulse - s * shake * 0.64;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 1.78 + s2 * cheek * 0.08;
      y = BILL_Y + Math.abs(s2) * shake * 1.12 + jaw * 0.08 * pulse;
      rot = s2 * shake * 1.55 + s * cheek * 0.095;
      ...
      // Past #47: still-under Look/laugh re-enter + punchier breathe / weight-shift.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 1.55;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 0.85;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 1.65 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 1.18 +
        Math.sin(t * rate * 0.17) * shift * 0.52;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 1.5 + Math.sin(t * rate * 1.7) * idle.scalePulse * 0.78);
      h = BILL_H * (1 + (b + b2) * 1.22);
      x = s;
      y = BILL_Y + b + b2 * 0.8;
      rot = s * 0.85;
