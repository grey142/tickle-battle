...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #59).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #59).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 16.2 : 2.2;
    ...
    const n = urls.length;
    // Desired bands (past #59): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 4 ? 0 : p < 11 ? 1 : p < 24 ? 2 : 3);
    ...
    // ±8.2 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 8.2;
    const upAt = [4 + dead, 11 + dead, 24 + dead];
    const downAt = [4 - dead, 11 - dead, 24 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.24 + pct * 0.0098)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #59 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 48 && span >= 2) {
      const cycle =
        pct >= 64
          ? [hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 32 && span >= 2) {
      // Soft peak lean: favor hi 7-of-8.
      idx = phase % 8 === 3 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
