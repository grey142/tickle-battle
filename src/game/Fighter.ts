...
export class Fighter {
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #167: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.0000000000000000000000000000000000000000000000000105) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.00000000000000000000000000000000000000000000000000163) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000000000000000000000000000000000000000000000000000227) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000000000000000000000000000000000000000000000000000208) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(397296, Math.round((stageParams?.billRate || 14) * (193.62 + (100 - pct) * 206.16)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #169).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #169).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 75.5 : 0.15;
    ...
    const n = urls.length;
    // Desired bands (past #169): soft f0–f1 holds longer → mid → hard/peak earlier (tighter).
    ...
    const bandFromPct = (p: number) => (p < 0.02 ? 0 : p < 0.22 ? 1 : p < 0.85 ? 2 : 3);
    ...
    // ±33.6 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 33.6;
    const upAt = [0.02 + dead, 0.22 + dead, 0.85 + dead];
    const downAt = [0.02 - dead, 0.22 - dead, 0.85 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #169 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 2.7 && span >= 2) {
      const cycle =
        pct >= 5.0
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 0.95 && span >= 2) {
      // Soft peak lean: favor hi 43-of-44.
      idx = phase % 44 === 22 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #167: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 4966.92 * pulse + s * shake * 26242.16;
      const morphH = 1 + jaw * 5693.17 * pulse - eye * 3784.15 * pulse - s * shake * 13844.22;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 35765.15 + s2 * cheek * 2066.99;
      y = BILL_Y + Math.abs(s2) * shake * 17159.57 + jaw * 2066.99 * pulse;
      rot = s2 * shake * 33579.68 + s * cheek * 2175.45;
