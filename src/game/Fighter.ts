...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #108: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 2.85;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #108: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 2.85;
  ...
  softenIdleSheetEnter(cap = 0.019) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #108: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.56;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #108: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 24);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #105: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.00000000046) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.000000000072) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.0000000000099) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.00000000000091) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(340, Math.round((stageParams?.billRate || 14) * (5.38 + (100 - pct) * 0.335)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #102).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #102).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 36.5 : 0.68;
    ...
    const n = urls.length;
    // Desired bands (past #102): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.32 ? 0 : p < 1.35 ? 1 : p < 4.4 ? 2 : 3);
    ...
    // ±17.8 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 17.8;
    const upAt = [0.32 + dead, 1.35 + dead, 4.4 + dead];
    const downAt = [0.32 - dead, 1.35 - dead, 4.4 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #102 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 10 && span >= 2) {
      const cycle =
        pct >= 22
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 5 && span >= 2) {
      // Soft peak lean: favor hi 17-of-18.
      idx = phase % 18 === 9 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #105: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 3.15 * pulse + s * shake * 16.66;
      const morphH = 1 + jaw * 3.61 * pulse - eye * 2.4 * pulse - s * shake * 8.78;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 22.69 + s2 * cheek * 1.31;
      y = BILL_Y + Math.abs(s2) * shake * 15.46 + jaw * 1.31 * pulse;
      rot = s2 * shake * 21.3 + s * cheek * 1.38;
      ...
      // Past #108: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 2.61;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.65;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 2.67 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 2.16 +
        Math.sin(t * rate * 0.17) * shift * 1.16;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.45 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.43);
      h = BILL_H * (1 + (b + b2) * 1.89);
      x = s;
      y = BILL_Y + b + b2 * 1.43;
      rot = s * 1.43;
