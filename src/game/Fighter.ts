...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #112: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #112: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 2.55;
  ...
  softenIdleSheetEnter(cap = 0.016) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #112: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.50;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #112: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 28);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Past #112: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 2.74;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.73;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 2.80 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 2.27 +
        Math.sin(t * rate * 0.17) * shift * 1.22;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.57 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.50);
      h = BILL_H * (1 + (b + b2) * 1.98);
      x = s;
      y = BILL_Y + b + b2 * 1.50;
      rot = s * 1.50;
