...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #134: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 1.08;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #134: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 1.08;
  ...
  softenIdleSheetEnter(cap = 0.0044) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #134: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.20;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #134: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 60);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #126).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #126).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 51.5 : 0.36;
    ...
    const n = urls.length;
    // Desired bands (past #126): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.12 ? 0 : p < 0.68 ? 1 : p < 2.4 ? 2 : 3);
    ...
    // ±24.0 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 24.0;
    const upAt = [0.12 + dead, 0.68 + dead, 2.4 + dead];
    const downAt = [0.12 - dead, 0.68 - dead, 2.4 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #126 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 5.3 && span >= 2) {
      const cycle =
        pct >= 11
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 2.5 && span >= 2) {
      // Soft peak lean: favor hi 27-of-28.
      idx = phase % 28 === 14 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Past #134: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 4.05;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 2.57;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 4.14 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 3.36 +
        Math.sin(t * rate * 0.17) * shift * 1.80;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 3.80 + Math.sin(t * rate * 1.7) * idle.scalePulse * 2.23);
      h = BILL_H * (1 + (b + b2) * 2.92);
      x = s;
      y = BILL_Y + b + b2 * 2.23;
      rot = s * 2.23;
