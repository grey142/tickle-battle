...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #57).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #57).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 14.8 : 2.5;
    ...
    const n = urls.length;
    // Desired bands (past #57): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 5 ? 0 : p < 13 ? 1 : p < 28 ? 2 : 3);
    ...
    // ±7.4 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 7.4;
    const upAt = [5 + dead, 13 + dead, 28 + dead];
    const downAt = [5 - dead, 13 - dead, 28 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.26 + pct * 0.0092)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #57 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 52 && span >= 2) {
      const cycle =
        pct >= 68
    ...
    } else if (pct >= 36 && span >= 2) {
      // Soft peak lean: favor hi 6-of-7.
      idx = phase % 7 === 3 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
