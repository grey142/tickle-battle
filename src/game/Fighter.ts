...
export class Fighter {
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #119: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.000000000000000031) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.00000000000000000485) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000000000000000000669) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000000000000000000616) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(1303, Math.round((stageParams?.billRate || 14) * (10.65 + (100 - pct) * 1.17)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #119: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 12.81 * pulse + s * shake * 67.75;
      const morphH = 1 + jaw * 14.68 * pulse - eye * 9.76 * pulse - s * shake * 35.71;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 92.26 + s2 * cheek * 5.33;
      y = BILL_Y + Math.abs(s2) * shake * 62.85 + jaw * 5.33 * pulse;
      rot = s2 * shake * 86.62 + s * cheek * 5.61;
