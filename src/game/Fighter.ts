...
export class Fighter {
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #62: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    const fps = Math.max(24, Math.round((stageParams?.billRate || 14) * (0.76 + (100 - pct) * 0.026)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #62: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.355 * pulse + s * shake * 1.9;
      const morphH = 1 + jaw * 0.41 * pulse - eye * 0.25 * pulse - s * shake * 1.0;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 2.6 + s2 * cheek * 0.14;
      y = BILL_Y + Math.abs(s2) * shake * 1.7 + jaw * 0.14 * pulse;
      rot = s2 * shake * 2.35 + s * cheek * 0.155;
