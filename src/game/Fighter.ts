...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #156: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 0.51;
  ...
  softenIdleSheetEnter(cap = 0.0014) {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #156: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 88);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #142).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #142).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 60.5 : 0.25;
    ...
    const n = urls.length;
    // Desired bands (past #142): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.07 ? 0 : p < 0.42 ? 1 : p < 1.6 ? 2 : 3);
    ...
    // ±27.6 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 27.6;
    const upAt = [0.07 + dead, 0.42 + dead, 1.6 + dead];
    const downAt = [0.07 - dead, 0.42 - dead, 1.6 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #142 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 3.7 && span >= 2) {
      const cycle =
        pct >= 7.0
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 1.7 && span >= 2) {
      // Soft peak lean: favor hi 33-of-34.
      idx = phase % 34 === 17 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      const b = Math.sin(t * rate) * idle.breatheAmp * 5.69;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 3.62;
