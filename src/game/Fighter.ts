...
export class Fighter {
  ...
  softenIdleSheetEnter(cap = 0.045) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #86: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.95;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #86: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 10);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #91: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    if (pct >= 0.00085) {
      ...
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000018) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000016) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(78, Math.round((stageParams?.billRate || 14) * (2.0 + (100 - pct) * 0.078)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #87).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #87).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 26.5 : 1.05;
    ...
    const n = urls.length;
    // Desired bands (past #87): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.9 ? 0 : p < 3.2 ? 1 : p < 8.8 ? 2 : 3);
    ...
    // ±13.0 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 13.0;
    const upAt = [0.9 + dead, 3.2 + dead, 8.8 + dead];
    const downAt = [0.9 - dead, 3.2 - dead, 8.8 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.12 + pct * 0.0142)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #87 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 24 && span >= 2) {
      const cycle =
        pct >= 40
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 12 && span >= 2) {
      // Soft peak lean: favor hi 13-of-14.
      idx = phase % 14 === 7 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #91: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.92 * pulse + s * shake * 4.85;
      const morphH = 1 + jaw * 1.05 * pulse - eye * 0.7 * pulse - s * shake * 2.55;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 6.6 + s2 * cheek * 0.38;
      y = BILL_Y + Math.abs(s2) * shake * 4.5 + jaw * 0.38 * pulse;
      rot = s2 * shake * 6.2 + s * cheek * 0.4;
      ...
      // Past #86: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 2.2;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.38;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 2.28 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 1.82 +
        Math.sin(t * rate * 0.17) * shift * 0.98;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.05 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.2);
      h = BILL_H * (1 + (b + b2) * 1.62);
      x = s;
      y = BILL_Y + b + b2 * 1.2;
      rot = s * 1.2;
