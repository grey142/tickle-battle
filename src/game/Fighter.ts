...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #160: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 0.413;
  ...
  softenIdleSheetEnter(cap = 0.001) {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #160: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 96);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #161: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.0000000000000000000000000000000000000000000404) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.00000000000000000000000000000000000000000000625) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000000000000000000000000000000000000000000000872) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000000000000000000000000000000000000000000000798) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(202702, Math.round((stageParams?.billRate || 14) * (137.66 + (100 - pct) * 112.29)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #155).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #155).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 69.5 : 0.19;
    ...
    const n = urls.length;
    // Desired bands (past #155): soft f0–f1 holds longer → mid → hard/peak earlier (tighter).
    ...
    const bandFromPct = (p: number) => (p < 0.04 ? 0 : p < 0.30 ? 1 : p < 1.15 ? 2 : 3);
    ...
    // ±31.2 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 31.2;
    const upAt = [0.04 + dead, 0.30 + dead, 1.15 + dead];
    const downAt = [0.04 - dead, 0.30 - dead, 1.15 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #155 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 3.1 && span >= 2) {
      const cycle =
        pct >= 5.8
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 1.25 && span >= 2) {
      // Soft peak lean: favor hi 39-of-40.
      idx = phase % 40 === 20 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #161: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 2465.83 * pulse + s * shake * 13027.95;
      const morphH = 1 + jaw * 2826.38 * pulse - eye * 1878.65 * pulse - s * shake * 6872.98;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 17755.65 + s2 * cheek * 1026.16;
      y = BILL_Y + Math.abs(s2) * shake * 8518.89 + jaw * 1026.16 * pulse;
      rot = s2 * shake * 16670.67 + s * cheek * 1080.01;
      ...
      // Past #150: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 6.27;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 3.99;
