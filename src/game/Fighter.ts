...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #116: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 2.05;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #116: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 2.05;
  ...
  softenIdleSheetEnter(cap = 0.012) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #116: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.40;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #116: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 36);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #117: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.00000000000000192) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.000000000000000301) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.0000000000000000415) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.00000000000000000382) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(931, Math.round((stageParams?.billRate || 14) * (8.98 + (100 - pct) * 0.86)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #109).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #109).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 42.5 : 0.50;
    ...
    const n = urls.length;
    // Desired bands (past #109): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.22 ? 0 : p < 1.0 ? 1 : p < 3.5 ? 2 : 3);
    ...
    // ±20.2 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 20.2;
    const upAt = [0.22 + dead, 1.0 + dead, 3.5 + dead];
    const downAt = [0.22 - dead, 1.0 - dead, 3.5 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #109 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 7.5 && span >= 2) {
      const cycle =
        pct >= 17
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 3.6 && span >= 2) {
      // Soft peak lean: favor hi 21-of-22.
      idx = phase % 22 === 11 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #117: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 9.02 * pulse + s * shake * 47.71;
      const morphH = 1 + jaw * 10.34 * pulse - eye * 6.87 * pulse - s * shake * 25.15;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 64.97 + s2 * cheek * 3.75;
      y = BILL_Y + Math.abs(s2) * shake * 44.26 + jaw * 3.75 * pulse;
      rot = s2 * shake * 61.0 + s * cheek * 3.95;
      ...
      // Past #116: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 3.02;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.91;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 3.09 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 2.50 +
        Math.sin(t * rate * 0.17) * shift * 1.34;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.84 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.66);
      h = BILL_H * (1 + (b + b2) * 2.18);
      x = s;
      y = BILL_Y + b + b2 * 1.66;
      rot = s * 1.66;
