...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #121: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 1.65;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #121: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 1.65;
  ...
  softenIdleSheetEnter(cap = 0.0085) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #121: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.32;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #121: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 44);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #115).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #115).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 45.5 : 0.44;
    ...
    const n = urls.length;
    // Desired bands (past #115): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.18 ? 0 : p < 0.88 ? 1 : p < 3.1 ? 2 : 3);
    ...
    // ±21.5 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 21.5;
    const upAt = [0.18 + dead, 0.88 + dead, 3.1 + dead];
    const downAt = [0.18 - dead, 0.88 - dead, 3.1 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #115 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 6.8 && span >= 2) {
      const cycle =
        pct >= 15
    ...
    } else if (pct >= 3.2 && span >= 2) {
      // Soft peak lean: favor hi 23-of-24.
      idx = phase % 24 === 12 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Past #121: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 3.33;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 2.11;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 3.40 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 2.76 +
        Math.sin(t * rate * 0.17) * shift * 1.48;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 3.13 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.83);
      h = BILL_H * (1 + (b + b2) * 2.40);
      x = s;
      y = BILL_Y + b + b2 * 1.83;
      rot = s * 1.83;
