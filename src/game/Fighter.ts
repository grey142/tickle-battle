...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #136: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 0.97;
  ...
  loadIdleSheet(slug?: string) {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      this.idleFrameAcc = 0;
      this.idleSheetEnter = 0;
      // Past #136: softer Look-edge ramp on fresh look sheet load.
      this.idleEnterRate = 0.97;
  ...
  softenIdleSheetEnter(cap = 0.0037) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #136: softer still-under ramp after laugh so the sheet eases back in.
    this.idleEnterRate = 0.18;
  }
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #136: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 64);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #138: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.00000000000000000000000000000882) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.00000000000000000000000000000137) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.000000000000000000000000000000190) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.0000000000000000000000000000000174) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(13735, Math.round((stageParams?.billRate || 14) * (35.16 + (100 - pct) * 9.88)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #130).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #130).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 54.5 : 0.32;
    ...
    const n = urls.length;
    // Desired bands (past #130): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 0.10 ? 0 : p < 0.58 ? 1 : p < 2.1 ? 2 : 3);
    ...
    // ±25.2 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 25.2;
    const upAt = [0.10 + dead, 0.58 + dead, 2.1 + dead];
    const downAt = [0.10 - dead, 0.58 - dead, 2.1 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #130 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 4.7 && span >= 2) {
      const cycle =
        pct >= 9.5
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 2.2 && span >= 2) {
      // Soft peak lean: favor hi 29-of-30.
      idx = phase % 30 === 15 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #138: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 149.16 * pulse + s * shake * 788.78;
      const morphH = 1 + jaw * 170.97 * pulse - eye * 113.64 * pulse - s * shake * 415.75;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 1074.06 + s2 * cheek * 62.08;
      y = BILL_Y + Math.abs(s2) * shake * 515.32 + jaw * 62.08 * pulse;
      rot = s2 * shake * 1008.43 + s * cheek * 65.33;
      ...
      // Past #136: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 4.25;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 2.70;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 4.35 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 3.53 +
        Math.sin(t * rate * 0.17) * shift * 1.89;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 3.99 + Math.sin(t * rate * 1.7) * idle.scalePulse * 2.34);
      h = BILL_H * (1 + (b + b2) * 3.07);
      x = s;
      y = BILL_Y + b + b2 * 2.34;
      rot = s * 2.34;
