...
export class Fighter {
  ...
  softenIdleSheetEnter(cap = 0.03) {
    this.idleSheetEnter = Math.min(this.idleSheetEnter, cap);
    // Past #98: softer still-under ramp after laugh so the sheet eases back in.
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #98: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 16);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #101: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    if (pct >= 0.000000029) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.0000000045) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.00000000062) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.000000000057) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(245, Math.round((stageParams?.billRate || 14) * (4.38 + (100 - pct) * 0.245)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #101: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 2.22 * pulse + s * shake * 11.73;
      const morphH = 1 + jaw * 2.54 * pulse - eye * 1.69 * pulse - s * shake * 6.18;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 15.98 + s2 * cheek * 0.92;
      y = BILL_Y + Math.abs(s2) * shake * 10.89 + jaw * 0.92 * pulse;
      rot = s2 * shake * 15.0 + s * cheek * 0.97;
      ...
      // Past #98: idle/skills/persist lane — crossfade / re-enter / sheet clarity. Persist profile-safe (no schema invent). Idle-scoped only.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 2.35;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 1.48;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 2.4 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 1.94 +
        Math.sin(t * rate * 0.17) * shift * 1.04;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 2.2 + Math.sin(t * rate * 1.7) * idle.scalePulse * 1.28);
      h = BILL_H * (1 + (b + b2) * 1.7);
      x = s;
      y = BILL_Y + b + b2 * 1.28;
      rot = s * 1.28;
