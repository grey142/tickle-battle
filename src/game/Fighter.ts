...
export class Fighter {
  ...
  applyLookSlug(slug: string, lookIdx: number) {
    ...
    // Past #158: softer Look-edge ramp on fresh look load (laugh soft-reenter stays via softenIdleSheetEnter).
    this.idleEnterRate = 0.459;
  ...
  softenIdleSheetEnter(cap = 0.0012) {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    // Past #158: smootherstep then higher-power plateau — longer mid-frame hold / sheet clarity.
    const s = u * u * u * (u * (u * 6 - 15) + 10);
    const blend = Math.pow(s, 92);
    this.idleFrame = f0;
    this.idleFrameAcc = u;
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #157: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    let lo = 0;
    let hi = n - 1;
    if (pct >= 0.00000000000000000000000000000000000000000250) {
      lo = 0;
      hi = Math.min(1, n - 1);
    } else if (pct >= 0.000000000000000000000000000000000000000000387) {
      lo = Math.min(1, n - 1);
      hi = Math.min(2, n - 1);
    } else if (pct >= 0.0000000000000000000000000000000000000000000540) {
      lo = Math.min(2, n - 1);
      hi = Math.min(3, n - 1);
    } else if (pct >= 0.00000000000000000000000000000000000000000000494) {
      lo = Math.min(3, n - 1);
      hi = n - 1;
    } else {
      lo = Math.min(4, n - 1);
    ...
    const fps = Math.max(144787, Math.round((stageParams?.billRate || 14) * (116.07 + (100 - pct) * 82.87)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #147).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #147).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 63.5 : 0.23;
    ...
    const n = urls.length;
    // Desired bands (past #147): soft f0–f1 holds longer → mid → hard/peak earlier (tighter).
    ...
    const bandFromPct = (p: number) => (p < 0.06 ? 0 : p < 0.38 ? 1 : p < 1.45 ? 2 : 3);
    ...
    // ±28.8 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 28.8;
    const upAt = [0.06 + dead, 0.38 + dead, 1.45 + dead];
    const downAt = [0.06 - dead, 0.38 - dead, 1.45 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    // Peak intensity: heavier f4 bias past #147 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 3.5 && span >= 2) {
      const cycle =
        pct >= 6.6
          ? [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo]
          : [hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 1.55 && span >= 2) {
      // Soft peak lean: favor hi 35-of-36.
      idx = phase % 36 === 18 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #157: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 1736.50 * pulse + s * shake * 9174.61;
      const morphH = 1 + jaw * 1990.41 * pulse - eye * 1322.99 * pulse - s * shake * 4840.13;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 12503.98 + s2 * cheek * 722.65;
      y = BILL_Y + Math.abs(s2) * shake * 5999.22 + jaw * 722.65 * pulse;
      rot = s2 * shake * 11739.91 + s * cheek * 760.57;
      ...
      const b = Math.sin(t * rate) * idle.breatheAmp * 5.97;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 3.8;
