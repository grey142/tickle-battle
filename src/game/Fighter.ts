...
export class Fighter {
  ...
  private applyIdleSheetFrame(dt: number, playing: boolean) {
    ...
    this.idleSheetEnter = Math.min(1, this.idleSheetEnter + dt * 3.4);
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #46: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    const fps = Math.max(15, Math.round((stageParams?.billRate || 14) * (0.44 + (100 - pct) * 0.0155)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #42).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #42).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 10.6 : 4.2;
    ...
    const n = urls.length;
    // Desired bands (past #42): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 10 ? 0 : p < 22 ? 1 : p < 40 ? 2 : 3);
    ...
    // ±5.0 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 5.0;
    const upAt = [10 + dead, 22 + dead, 40 + dead];
    const downAt = [10 - dead, 22 - dead, 40 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.32 + pct * 0.0074)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #42 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 64 && span >= 2) {
      const cycle =
        pct >= 80
          ? [hi, hi, hi, hi, lo, hi, hi, hi]
          : [hi, hi, hi, lo, hi, hi, lo, hi];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 48 && span >= 2) {
      // Soft peak lean: favor hi 3-of-4.
      idx = phase % 4 === 2 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #46: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.17 * pulse + s * shake * 1.08;
      const morphH = 1 + jaw * 0.2 * pulse - eye * 0.12 * pulse - s * shake * 0.52;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 1.5 + s2 * cheek * 0.06;
      y = BILL_Y + Math.abs(s2) * shake * 0.9 + jaw * 0.06 * pulse;
      rot = s2 * shake * 1.32 + s * cheek * 0.075;
      ...
      const rate = loco?.billRate ?? (clip === "run" ? 16.25 : 10.3);
      const bob = loco?.billBob ?? (clip === "run" ? 0.094 : 0.044);
      const s = Math.sin(t * rate);
      const s2 = Math.sin(t * rate * 2);
      // v3 deepen past #41: punchier stride bob / sway / squash on billboards.
      w = BILL_W * (1 + s * bob * 0.86 + Math.abs(s2) * bob * 0.28);
      h = BILL_H * (1 + Math.abs(s) * bob * 0.66 - Math.abs(s2) * bob * 0.34);
      x = s * bob * 1.12;
      y = BILL_Y + Math.abs(s) * bob * 2.0;
      rot = s * bob * 1.18;
    } else {
      // Past #43: still-under Look enter + punchier breathe / weight-shift.
      this.applyIdleSheetFrame(dt, true);
      const idle = idleBindForSlug(this.slug);
      const rate = idle.breatheRate;
      const b = Math.sin(t * rate) * idle.breatheAmp * 1.48;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * idle.breatheAmp * 0.78;
      const shift = idle.weightShift ?? idle.sway;
      const s =
        Math.sin(t * rate * 0.62) * idle.sway * 1.55 +
        Math.sin(t * rate * 0.31 + 0.4) * shift * 1.1 +
        Math.sin(t * rate * 0.17) * shift * 0.48;
      w = BILL_W * (1 + Math.sin(t * rate) * idle.scalePulse * 1.42 + Math.sin(t * rate * 1.7) * idle.scalePulse * 0.7);
      h = BILL_H * (1 + (b + b2) * 1.18);
      x = s;
      y = BILL_Y + b + b2 * 0.75;
      rot = s * 0.8;
