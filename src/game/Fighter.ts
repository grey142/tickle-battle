...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  private syncTickleFrameBillboard(clip: string, tickle?: TickleBind, dt = 0.016) {
    ...
    // Intensity-weighted window: victim stamina → harder f3/f4 (clearer past #51).
    ...
    // Asymmetric smooth: climb into hard windows faster; ease out slower (past #51).
    const climb = target > this.tickleIntensitySmooth;
    const rate = climb ? 13.2 : 3.0;
    ...
    const n = urls.length;
    // Desired bands (past #51): soft f0–f1 holds longer → mid → hard/peak earlier.
    ...
    const bandFromPct = (p: number) => (p < 6 ? 0 : p < 15 ? 1 : p < 32 ? 2 : 3);
    ...
    // ±6.6 deadband so edge flicker doesn't thrash lo/hi.
    const dead = 6.6;
    const upAt = [6 + dead, 15 + dead, 32 + dead];
    const downAt = [6 - dead, 15 - dead, 32 - dead];
    let band = prevBand;
    if (rawBand > prevBand) {
    ...
    const fps = Math.max(10, Math.round((tickle.billRate || 26) * (0.28 + pct * 0.0086)));
    const phase = Math.floor(this.animT * fps);
    // Peak intensity: heavier f4 bias past #51 (ultra locks f4 hard).
    let idx: number;
    if (pct >= 56 && span >= 2) {
      const cycle =
        pct >= 72
          ? [hi, hi, hi, hi, hi, hi, lo, hi]
          : [hi, hi, hi, hi, hi, lo, hi, lo];
      idx = cycle[phase % cycle.length];
    } else if (pct >= 40 && span >= 2) {
      // Soft peak lean: favor hi 5-of-6.
      idx = phase % 6 === 3 ? lo : hi;
    } else {
      idx = lo + (phase % span);
    }
