...
export class Fighter {
  ...
  private syncLaughFrameBillboard(
    ...
    // Tighter stamina→frame past #77: high soft-locks f0–f1 earlier; mid f1–f2;
    ...
    const fps = Math.max(48, Math.round((stageParams?.billRate || 14) * (1.42 + (100 - pct) * 0.05)));
...
/** Cycle AI billboard (and FP portrait URL) through tickle frames while tickling. */
  ...
  private tickBillboardAnim(
    ...
    if (this.occupancy === "tickler" || (this.occupancy === "nudge" && this.joinOn >= 0)) {
      ...
      // Billboard morph-feel past #77: cheek widens, jaw opens (taller), eye squint shortens.
      const morphW = 1 + cheek * 0.63 * pulse + s * shake * 3.3;
      const morphH = 1 + jaw * 0.71 * pulse - eye * 0.46 * pulse - s * shake * 1.72;
      w = BILL_W * morphW;
      h = BILL_H * morphH;
      x = s * shake * 4.45 + s2 * cheek * 0.255;
      y = BILL_Y + Math.abs(s2) * shake * 3.0 + jaw * 0.255 * pulse;
      rot = s2 * shake * 4.1 + s * cheek * 0.27;
