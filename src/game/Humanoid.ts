...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 11.66;
      const lean = (loco?.lean ?? 0.12) * 11.38;
      ...
      // Clearer head tilt for painterly morph past #117 (stable — no wild clipping).
      this.head.rotation.set(
        2.66 + s2 * lean * 9.86 + Math.abs(s4) * 1.138,
        s * lean * 10.54,
        s3 * lean * 6.12 + s2 * 0.549,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #117).
      this.head.scale.set(
        1 + cheek * 2.93 * laughPulse,
        1 + jaw * 1.95 * laughPulse,
        1 + cheek * 1.147 + eye * 0.515,
      );
      ...
      this.face.scale.set(
        1 + cheek * 2.33 * laughPulse,
        1 + jawOpen * 11.6 - eyeSquint * 4.62,
        1,
      );
      this.face.position.set(
        s * cheek * 0.252,
        0.216 + brow * 0.935 * laughPulse - jawOpen * 0.555,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 1.226 + s * 0.319;
