...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 18.51;
      const lean = (loco?.lean ?? 0.12) * 18.06;
      ...
      // Clearer head tilt for painterly morph past #133 (stable — no wild clipping).
      this.head.rotation.set(
        4.22 + s2 * lean * 15.64 + Math.abs(s4) * 1.806,
        s * lean * 16.72,
        s3 * lean * 9.72 + s2 * 0.87,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #133).
      this.head.scale.set(
        1 + cheek * 4.63 * laughPulse,
        1 + jaw * 3.10 * laughPulse,
        1 + cheek * 1.821 + eye * 0.816,
      );
      ...
      this.face.scale.set(
        1 + cheek * 3.70 * laughPulse,
        1 + jawOpen * 18.4 - eyeSquint * 7.33,
        1,
      );
      this.face.position.set(
        s * cheek * 0.400,
        0.343 + brow * 1.48 * laughPulse - jawOpen * 0.88,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 1.945 + s * 0.508;
