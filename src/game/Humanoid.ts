...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "idle") {
      ...
      const rate = 1.9;
      const b = Math.sin(t * rate) * 0.045;
      const b2 = Math.sin(t * rate * 0.53 + 0.7) * 0.022;
      const shift = Math.sin(t * rate * 0.35) * 0.038 + Math.sin(t * rate * 0.18) * 0.016;
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 2.12;
      const lean = (loco?.lean ?? 0.12) * 1.95;
      ...
      // Clearer head tilt for painterly morph past #44 (stable — no wild clipping).
      this.head.rotation.set(
        0.52 + s2 * lean * 1.82 + Math.abs(s4) * 0.17,
        s * lean * 1.98,
        s3 * lean * 1.05 + s2 * 0.08,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #44).
      this.head.scale.set(
        1 + cheek * 0.44 * laughPulse,
        1 + jaw * 0.29 * laughPulse,
        1 + cheek * 0.165 + eye * 0.07,
      );
      ...
      this.face.scale.set(
        ...
        1 + jawOpen * 1.78 - eyeSquint * 0.68,
        1,
      );
      this.face.position.set(
        s * cheek * 0.028,
        0.032 + brow * 0.13 * laughPulse - jawOpen * 0.08,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 0.175 + s * 0.04;
