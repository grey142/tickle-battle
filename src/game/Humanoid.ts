...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 4.25;
      const lean = (loco?.lean ?? 0.12) * 4.05;
      ...
      // Clearer head tilt for painterly morph past #66 (stable — no wild clipping).
      this.head.rotation.set(
        1.12 + s2 * lean * 3.65 + Math.abs(s4) * 0.41,
        s * lean * 3.8,
        s3 * lean * 2.2 + s2 * 0.195,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #66).
      this.head.scale.set(
        1 + cheek * 1.04 * laughPulse,
        1 + jaw * 0.71 * laughPulse,
        1 + cheek * 0.405 + eye * 0.18,
      );
      ...
      this.face.scale.set(
        1 + cheek * 0.82 * laughPulse,
        1 + jawOpen * 4.1 - eyeSquint * 1.65,
        1,
      );
      this.face.position.set(
        s * cheek * 0.085,
        0.076 + brow * 0.33 * laughPulse - jawOpen * 0.2,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 0.435 + s * 0.112;
