...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 9.2;
      const lean = (loco?.lean ?? 0.12) * 9.0;
      ...
      // Clearer head tilt for painterly morph past #105 (stable — no wild clipping).
      this.head.rotation.set(
        2.11 + s2 * lean * 7.82 + Math.abs(s4) * 0.9,
        s * lean * 8.37,
        s3 * lean * 4.86 + s2 * 0.435,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #105).
      this.head.scale.set(
        1 + cheek * 2.32 * laughPulse,
        1 + jaw * 1.56 * laughPulse,
        1 + cheek * 0.91 + eye * 0.409,
      );
      ...
      this.face.scale.set(
        1 + cheek * 1.85 * laughPulse,
        1 + jawOpen * 9.2 - eyeSquint * 3.67,
        1,
      );
      this.face.position.set(
        s * cheek * 0.2,
        0.171 + brow * 0.743 * laughPulse - jawOpen * 0.441,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 0.973 + s * 0.253;
