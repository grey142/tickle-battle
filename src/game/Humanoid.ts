...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 12.59;
      const lean = (loco?.lean ?? 0.12) * 12.29;
      ...
      // Clearer head tilt for painterly morph past #119 (stable — no wild clipping).
      this.head.rotation.set(
        2.87 + s2 * lean * 10.65 + Math.abs(s4) * 1.229,
        s * lean * 11.38,
        s3 * lean * 6.61 + s2 * 0.593,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #119).
      this.head.scale.set(
        1 + cheek * 3.16 * laughPulse,
        1 + jaw * 2.11 * laughPulse,
        1 + cheek * 1.239 + eye * 0.556,
      );
      ...
      this.face.scale.set(
        1 + cheek * 2.52 * laughPulse,
        1 + jawOpen * 12.53 - eyeSquint * 4.99,
        1,
      );
      this.face.position.set(
        s * cheek * 0.272,
        0.233 + brow * 1.01 * laughPulse - jawOpen * 0.599,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 1.324 + s * 0.345;
