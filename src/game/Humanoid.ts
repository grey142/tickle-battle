...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 19.99;
      const lean = (loco?.lean ?? 0.12) * 19.50;
      ...
      // Clearer head tilt for painterly morph past #135 (stable — no wild clipping).
      this.head.rotation.set(
        4.56 + s2 * lean * 16.89 + Math.abs(s4) * 1.950,
        s * lean * 18.06,
        s3 * lean * 10.50 + s2 * 0.94,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #135).
      this.head.scale.set(
        1 + cheek * 5.00 * laughPulse,
        1 + jaw * 3.35 * laughPulse,
        1 + cheek * 1.966 + eye * 0.881,
      );
      ...
      this.face.scale.set(
        1 + cheek * 4.00 * laughPulse,
        1 + jawOpen * 19.87 - eyeSquint * 7.92,
        1,
      );
      this.face.position.set(
        s * cheek * 0.432,
        0.370 + brow * 1.60 * laughPulse - jawOpen * 0.95,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 2.101 + s * 0.549;
