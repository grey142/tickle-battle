...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 25.19;
      const lean = (loco?.lean ?? 0.12) * 24.56;
      ...
      // Clearer head tilt for painterly morph past #141 (stable — no wild clipping).
      this.head.rotation.set(
        5.73 + s2 * lean * 21.28 + Math.abs(s4) * 2.456,
        s * lean * 22.74,
        s3 * lean * 13.23 + s2 * 1.19,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #141).
      this.head.scale.set(
        1 + cheek * 6.30 * laughPulse,
        1 + jaw * 4.22 * laughPulse,
        1 + cheek * 2.476 + eye * 1.109,
      );
      ...
      this.face.scale.set(
        1 + cheek * 5.04 * laughPulse,
        1 + jawOpen * 25.03 - eyeSquint * 9.97,
        1,
      );
      this.face.position.set(
        s * cheek * 0.544,
        0.467 + brow * 2.02 * laughPulse - jawOpen * 1.2,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 2.647 + s * 0.691;
