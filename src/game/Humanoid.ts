...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 5.4;
      const lean = (loco?.lean ?? 0.12) * 5.2;
      ...
      // Clearer head tilt for painterly morph past #77 (stable — no wild clipping).
      this.head.rotation.set(
        1.36 + s2 * lean * 4.65 + Math.abs(s4) * 0.53,
        s * lean * 4.9,
        s3 * lean * 2.85 + s2 * 0.255,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #77).
      this.head.scale.set(
        1 + cheek * 1.35 * laughPulse,
        1 + jaw * 0.92 * laughPulse,
        1 + cheek * 0.53 + eye * 0.24,
      );
      ...
      this.face.scale.set(
        1 + cheek * 1.07 * laughPulse,
        1 + jawOpen * 5.35 - eyeSquint * 2.15,
        1,
      );
      this.face.position.set(
        s * cheek * 0.115,
        0.1 + brow * 0.435 * laughPulse - jawOpen * 0.26,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 0.57 + s * 0.148;
