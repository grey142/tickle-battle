...
export class Humanoid {
  ...
  pose(clip: Clip, t: number, mods?: PoseMods) {
    ...
    if (clip === "squirm") {
      ...
      const amp = (loco?.amp ?? 0.1) * 5.85;
      const lean = (loco?.lean ?? 0.12) * 5.65;
      ...
      // Clearer head tilt for painterly morph past #81 (stable — no wild clipping).
      this.head.rotation.set(
        1.44 + s2 * lean * 5.0 + Math.abs(s4) * 0.57,
        s * lean * 5.3,
        s3 * lean * 3.08 + s2 * 0.275,
      );
      // Soft head morph toward painterly/morph-target feel (capsule kit; another notch past #81).
      this.head.scale.set(
        1 + cheek * 1.46 * laughPulse,
        1 + jaw * 0.99 * laughPulse,
        1 + cheek * 0.575 + eye * 0.26,
      );
      ...
      this.face.scale.set(
        1 + cheek * 1.16 * laughPulse,
        1 + jawOpen * 5.8 - eyeSquint * 2.32,
        1,
      );
      this.face.position.set(
        s * cheek * 0.125,
        0.108 + brow * 0.47 * laughPulse - jawOpen * 0.28,
        -0.128,
      );
      this.face.rotation.z = s2 * cheek * 0.615 + s * 0.16;
