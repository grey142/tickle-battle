...
past #84. Full painterly production run art is still deferred.
...
def make_frames(src: Path) -> list[Image.Image]:
    """v14 deepen past #84: punchier opposite-leg f1/f2 + peak f3 stride + oil grade."""
    ...
    # f1 — plant / opposite sway (clearer opposite-leg punch past #84)
    f1 = punch_stride(base, 3.75)
    f1 = affine_frame(f1, 0.246, -0.073, 1.124, 0.806, -hshift(base, 0.092), -7.6)
    f1 = ImageEnhance.Contrast(f1).enhance(1.26)
    f1 = cool_grade(f1, 2.24)
    f1 = painterly(f1, 1.04)
    f1 = vignette(f1, 1.58)

    # f2 — opposite plant / stronger extension
    f2 = punch_stride(base, 4.8)
    f2 = affine_frame(f2, -0.284, 0.094, 1.2, 0.712, -hshift(base, 0.132), 8.9)
    f2 = ImageEnhance.Brightness(f2).enhance(1.085)
    f2 = ImageEnhance.Contrast(f2).enhance(1.3)
    f2 = cool_grade(f2, 2.78)
    f2 = painterly(f2, 1.26)
    f2 = f2.filter(ImageFilter.UnsharpMask(radius=2.1, percent=175, threshold=2))
    f2 = vignette(f2, 2.02)

    # f3 — peak bob / squash / forward lean
    f3 = punch_stride(base, 4.6)
    f3 = affine_frame(f3, 0.19, -0.104, 1.254, 0.654, -hshift(base, 0.17), 4.6)
    f3 = ImageEnhance.Sharpness(f3).enhance(1.5)
    f3 = ImageEnhance.Contrast(f3).enhance(1.32)
    f3 = cool_grade(f3, 3.55)
    f3 = painterly(f3, 1.46)
...
def main() -> None:
    ...
    manifest = {
        ...
        "note": (
            ...
            "+ oil-paint grade past #84. Not full painterly. Elara jewelry exception only."
        ),
    }
