...
past #92. Full painterly production run art is still deferred.
...
def make_frames(src: Path) -> list[Image.Image]:
    """v16 deepen past #92: punchier opposite-leg f1/f2 + peak f3 stride + oil grade."""
    ...
    # f1 — plant / opposite sway (clearer opposite-leg punch past #92)
    f1 = punch_stride(base, 4.25)
    f1 = affine_frame(f1, 0.27, -0.083, 1.155, 0.786, -hshift(base, 0.108), -8.4)
    f1 = ImageEnhance.Contrast(f1).enhance(1.3)
    f1 = cool_grade(f1, 2.48)
    f1 = painterly(f1, 1.2)
    f1 = vignette(f1, 1.82)

    # f2 — opposite plant / stronger extension
    f2 = punch_stride(base, 5.45)
    f2 = affine_frame(f2, -0.308, 0.106, 1.24, 0.688, -hshift(base, 0.154), 9.9)
    f2 = ImageEnhance.Brightness(f2).enhance(1.095)
    f2 = ImageEnhance.Contrast(f2).enhance(1.36)
    f2 = cool_grade(f2, 3.1)
    f2 = painterly(f2, 1.46)
    f2 = f2.filter(ImageFilter.UnsharpMask(radius=2.1, percent=190, threshold=2))
    f2 = vignette(f2, 2.3)

    # f3 — peak bob / squash / forward lean
    f3 = punch_stride(base, 5.2)
    f3 = affine_frame(f3, 0.21, -0.12, 1.296, 0.626, -hshift(base, 0.194), 5.2)
    f3 = ImageEnhance.Sharpness(f3).enhance(1.62)
    f3 = ImageEnhance.Contrast(f3).enhance(1.37)
    f3 = cool_grade(f3, 3.9)
    f3 = painterly(f3, 1.7)
    f3 = f3.filter(ImageFilter.UnsharpMask(radius=2.35, percent=206, threshold=2))
    f3 = vignette(f3, 2.56)
    return [f0, f1, f2, f3]
...
def main() -> None:
    ...
    manifest = {
        ...
        "note": (
            ...
            "+ oil-paint grade past #92. Not full painterly. Elara jewelry exception only."
        ),
    }
